...
This page connects a running JupyterLab instance to GitHub so postdocs and collaborators can clone the postdoc project repository and move small project files between the instance and GitHub.

GitHub is where this postdoc project keeps code, Markdown pages, notebooks, meeting notes, small figures, and public website content. Persistent storage is where large data and large outputs belong. The compute instance is where active work happens.

Before using this page, review the [Cloud Triangle guide](../resources/cloud-triangle.md).

## Why The Instance Needs GitHub Access

The JupyterLab instance is useful for analysis, but it is not the long-term record of the postdoc project. Connecting it to GitHub lets postdocs and collaborators:
...
For most postdocs and collaborators, the recommended path is GitHub web authentication with an HTTPS clone link. This avoids managing SSH keys in a temporary compute environment.

## Step 1: Launch The JupyterLab Instance

Open the postdoc project JupyterLab or CyVerse VICE app provided by ESIIL staff or your facilitator. Wait for the instance to finish starting.
...
## Step 5: Clone The Postdoc Project Repository
...
```text
https://github.com/CU-ESIIL/Postdoc_OASIS.git
```
...
Start each work session by pulling the latest changes. Postdoc projects often edit over weeks or months, so someone else may have updated the repository since your last session.
...
SSH can still work for postdocs and collaborators who already know how to manage SSH keys in temporary JupyterLab environments, but it is not the recommended first path here.
...
```text
git@github.com:CU-ESIIL/Postdoc_OASIS.git
```
