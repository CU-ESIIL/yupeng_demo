...
Postdoc projects create materials that others may want to understand, cite, adapt, or build on. This page helps the postdoc and collaborators make outputs reusable without making the repository harder to maintain.
...
If the repository uses a bibliography file, add references there and cite them in Markdown using the repository convention. If no bibliography file exists yet, keep citation notes in a simple Markdown references section until the postdoc decides on a citation workflow.
...
```text
Reuse note: This figure is a final postdoc project output. Please cite the postdoc project website and the archived dataset listed in the figure caption. Source notebook: notebooks/figure-01.ipynb. Large intermediate files are stored in persistent storage with a README describing provenance and processing.
```
